...
const LegalHub: NextPage = () => {
    ...
    return (
        <motion.div
            ...
            <NextSeo title="Legal Information" description="Legal documents and policies for A Thing" />
            ...
            <div className="flex w-full max-w-4xl flex-col gap-8">
                <div className="flex flex-col gap-4 border-2 bg-white p-8 hover:border-black">
                    ...
                    <p className="text-gray-600">
                        Our commitment to transparency means all our policies are written in plain language.
                        We believe you should understand exactly how A Thing works and how your data is handled.
                    </p>
                </div>
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                    {legalDocuments.map((doc) => (
                        ...
                        >
                            <motion.div 
                                className={`flex h-full flex-col gap-2 border-2 bg-white p-6 hover:border-black
                                    ${doc.priority === 'high' ? 'border-black' : 'border-gray-200'}`}
                                whileHover={{ scale: 1.02 }}
                                transition={{ duration: 0.2 }}>
                                <h2 className="text-xl font-bold">{doc.title}</h2>
                                <p className="text-sm text-gray-600">{doc.description}</p>
                                {doc.external && (
                                    <span className="mt-2 text-xs text-gray-500">Opens in new tab ↗</span>
                                )}
                            </motion.div>
                        </Link>
                    ))}
